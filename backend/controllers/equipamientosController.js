...
module.exports = {
  getAll: async function (req, res, next) {
    try {
      const obras = await equipamientoModel.find();
  ...
  getById: async function (req, res, next) {
    try {
      const obra = await equipamientoModel.findById(req.params.id);
  ...
  update: async function (req, res, next) {
    ...
    let dataToBeUpdated = "";
    try {
      dataToBeUpdated = await equipamientoModel.findById({ _id: id });
    } catch (e) {
      console.log(e);
      return res.status(500).send({ error: true, message: "Couldn´t access to DB." });
    }
    ...
    try {
      await equipamientoModel.updateOne({ _id: req.params.id }, dataToBeUpdated);
      let updatedObra = await equipamientoModel.findById({ _id: id });
